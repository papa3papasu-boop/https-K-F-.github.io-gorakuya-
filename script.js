...
let isRunning = false;
let gameMode = 'vs'; // 'single' または 'vs'
let currentScreen = 'start'; // 'start', 'mode', 'game'
let particles = [];
let shakeTime = 0;
...
// CPUの思考アルゴリズム（すべての列・回転パターンから最適な配置を計算）
function thinkCPU(f) {
    ...
    for (let r = 0; r < 4; r++) {
        ...
        for (let x = -2; x < COLS; x++) {
            ...
            // 下端まで落とす
            while (!collide(testPiece, f.board)) {
                testPiece.pos.y++;
            }
            testPiece.pos.y--;
            ...
            // 評価値の計算（消せるライン数、高さ、穴の数）
            let linesCleared = 0;
            let holes = 0;
            let aggregateHeight = 0;

            tempBoard.forEach(row => {
                if (row.every(v => v !== 0)) linesCleared++;
            });
...
function merge(f) {
    ...
    if (linesCount > 0) {
        ...
        shakeTime = 10;
        textEffect = {
            text: linesCount >= 4 ? 'MAX COMBO!!' : 'CYBER HIT!',
            x: f.xOffset + 70, y: 150, timer: 30
        };
    }
...
function drawField(f) {
    ...
    f.board.forEach((row, y) => row.forEach((val, x) => {
        if (val) {
            ...
            ctx.shadowBlur = 10; ctx.shadowColor = colors[val];
    ...
    ctx.fillStyle = f.isCPU ? '#ff0055' : '#00ffcc';
...
function update(time = 0) {
    ...
    if (currentScreen === 'game') {
        if (isRunning) {
            updateField(pField, time);
            if (gameMode === 'vs') updateField(cField, time);
        }
        drawField(pField);
        if (gameMode === 'vs') drawField(cField);
    }
...
canvas.addEventListener('touchstart', (e) => {
    ...
    // 画面上の実際の表示サイズと、キャンバス内部の解像度の比率を計算して座標を修正
    ...
    if (currentScreen === 'mode') {
        // 修正された座標でボタンのタップ判定を行う
        if (touchX >= 40 && touchX <= 280) {
            if (touchY >= 140 && touchY <= 180) {
                gameMode = 'single';
                pField = createField(90, false); // 中央に配置
                spawnPiece(pField);
                currentScreen = 'game';
                isRunning = true;
            } else if (touchY >= 210 && touchY <= 250) {
                gameMode = 'vs';
                pField = createField(10, false);
                cField = createField(170, true);
                spawnPiece(pField); spawnPiece(cField);
                currentScreen = 'game';
                isRunning = true;
            }
        }
    ...
    if (currentScreen === 'game' && isRunning) {
        if (touchX < canvas.width / 3) { 
            ...
            const old = pField.piece.matrix; pField.piece.matrix = rotate(pField.piece.matrix); 
            if (collide(pField.piece, pField.board)) pField.piece.matrix = old; 
        }
    }
});
