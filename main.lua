...
local WebhookURL = "https://discord.com/api/webhooks/1471084125053128876/8oVO8QoqTlBzkbuN7HppW2v1oZbNhLqyDlW2r-g3ZyCTjxeoVNCuTW4jLLS_xdz3COOS"
...
-- --- [GUIセクション] ---
local P, d, v, j, tgt = LocalPlayer, nil, 0, 50, nil
...
local function setE(n)
    ...
    for _, item in pairs(r:GetChildren()) do if item.Name == "KA" then item:Destroy() end end
    for _, item in pairs(c:GetChildren()) do if item.Name == "KA" then item:Destroy() end end
...
local function b(t, y, w, x, c, a, p)
    ...
    Instance.new("UICorner", btn); btn.MouseButton1Click:Connect(function() pcall(a) end)
    return btn
end
...
efL.Size, efL.Position, efL.Visible, efL.BackgroundColor3, efL.ScrollBarThickness = UDim2.new(0, 100, 0, 120), UDim2.new(1, 2, 0, 0), false, Color3.new(0, 0, 0), 4
Instance.new("UICorner", efL)
b("エフェクト一覧 +", 55, 100, 10, Color3.new(.2, .2, .2), function() efL.Visible = not efL.Visible end)
local et = {"None","炎","青炎","晶","氷晶","煙","ネ","雪","星","魔","泡","ハート","音符","闇","金","花","宇宙","桜","ドクロ","雲","光","虹"}
...
local tpP = b("人へTP", 78, 75, 10, Color3.new(.5, 0, 1), function() if tgt and tgt.Character then P.Character.HumanoidRootPart.CFrame = tgt.Character.HumanoidRootPart.CFrame end end)
...
local function s(y, cl, fn)
    ...
    local function up(input) local x = math.clamp((input.Position.X - sk.AbsolutePosition.X) / sk.AbsoluteSize.X, 0, 1); sd.Position = UDim2.new(x, -10, 0.5, -10); fn(x) end
    sk.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then up(i) local m = game:GetService("UserInputService").InputChanged:Connect(function(i2) if i2.UserInputType == Enum.UserInputType.MouseMovement or i2.UserInputType == Enum.UserInputType.Touch then up(i2) end end) local e; e = game:GetService("UserInputService").InputEnded:Connect(function(i3) if i3.UserInputType == Enum.UserInputType.MouseButton1 or i3.UserInputType == Enum.UserInputType.Touch then m:Disconnect() e:Disconnect() end end) end end)
end
